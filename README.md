...
A drawer of free little tools. No accounts, no credit cards, no ads. Most tools run entirely on the user's device; a few use AI and are clearly labeled.

## Architecture

- **Local-first / static.** Core tools are 100% client-side with no backend - their logic runs in the user's browser. AI tools may use a Cloudflare Pages Function proxy to a labeled third-party service (see CLAUDE.md).
