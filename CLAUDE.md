...
- **Same palette + voice across every page.** The site is mid-migration to the **"Flow" redesign** (approved by the founder 2026-09-24; handoff in `Techtuate redesign directions/`): dark `#06050a` background, animated WebGL silk backdrop (Ember mood), frosted-glass panels, yellow `#ffd60a` accent, Bricolage Grotesque / Instrument Serif italic / Geist / Geist Mono, self-hosted in `/assets/fonts/`. Migrated pages link `/assets/flow.css` + `/assets/backdrop.js` (NOT `/assets/site.css`): so far `/` (plus `/assets/home.css` + `/assets/home.js`) and `/card-reader/`. Every other page still uses the old white/yellow neo-brutalist `/assets/site.css` until it is moved over; when the last page moves, delete site.css. New or restyled pages use the Flow shell (nav, backdrop, footer, FAQ accordion). Mobile-friendly down to ~360px. **Approved exception (2026-08-04):** the `/card-reader/` reach-out actions use real brand colors (WhatsApp green, Gmail red, Outlook/LinkedIn blue) for instant recognizability. This is a deliberate, scoped exception - do not "correct" them. No other page introduces off-palette color without founder sign-off.
...
2. **Wiring checklist - do ALL of these, every time (this is the step that gets missed):**
   ...
   - [ ] `index.html` - add an `<a class="tool">` card to the landing grid (copy an existing one). Its `data-kw` (search synonyms), `data-short`, `data-groups` (ids from `GROUPS` in `/assets/home.js`), `data-ai="1"` (AI tools only) and `--h1/--h2` hues drive search, grouping and the device-only toggle; the tool count updates itself. Also add it to the ItemList JSON-LD in the same file.
